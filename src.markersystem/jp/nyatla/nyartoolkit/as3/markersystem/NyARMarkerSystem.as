...
package jp.nyatla.nyartoolkit.as3.markersystem
{
	...
	public class NyARMarkerSystem
	{
		...
		private var _rledetect:RleDetector;
		...
		public function getConfidence(i_id:int):Number
		{
			if((i_id & MASK_IDTYPE)==IDTYPE_ARTK){
				//ARマーカ
				return MarkerInfoARMarker(this._rledetect._armk_list.getItem(i_id &MASK_IDNUM)).cf;
			}
			//Idマーカ？
			throw new NyARException();
		}
		...
		public function getNyId(i_id:int):Number
		{
			if((i_id & MASK_IDTYPE)==IDTYPE_NYID){
				//Idマーカ
				return MarkerInfoNyId(this._rledetect._idmk_list.getItem(i_id &MASK_IDNUM)).nyid;
			}
			//ARマーカ？
			throw new NyARException();
		}
		...
		public function getLife(i_id:int):int
		{
			if((i_id & MASK_IDTYPE)==IDTYPE_ARTK){
				//ARマーカ
				return MarkerInfoARMarker(this._rledetect._armk_list.getItem(i_id & MASK_IDNUM)).life;
			}else{
				//Idマーカ
				return MarkerInfoNyId(this._rledetect._idmk_list.getItem(i_id & MASK_IDNUM)).life;
			}
		}
		/**
		...
		public function getLostCount(i_id:int):int
		{
			if((i_id & MASK_IDTYPE)==IDTYPE_ARTK){
				//ARマーカ
				return MarkerInfoARMarker(this._rledetect._armk_list.getItem(i_id & MASK_IDNUM)).lost_count;
			}else{
				//Idマーカ
				return MarkerInfoNyId(this._rledetect._idmk_list.getItem(i_id & MASK_IDNUM)).lost_count;
			}
		}
		/**
		...
		public function getScreenPos(i_id:int,i_x:Number,i_y:Number,i_z:Number,i_out:NyARDoublePoint2d):NyARDoublePoint2d
		{
			...
			this.getMarkerMatrix(i_id).transform3d_1(i_x, i_y, i_z,_wk_3dpos);
			...
			i_raster:INyARRgbRaster):INyARRgbRaster
		{
			...
			tmat.transform3d_1(i_x1, i_y1,0,	pos[1]);
			tmat.transform3d_1(i_x2, i_y2,0,	pos[0]);
			tmat.transform3d_1(i_x3, i_y3,0,	pos[3]);
			tmat.transform3d_1(i_x4, i_y4,0,	pos[2]);
		...
		public function getMarkerMatrix(i_id:int):NyARDoubleMatrix44
		{
			if((i_id & MASK_IDTYPE)==IDTYPE_ARTK){
				//ARマーカ
				return MarkerInfoARMarker(this._rledetect._armk_list.getItem(i_id &MASK_IDNUM)).tmat;
			}else{
				//Idマーカ
				return MarkerInfoNyId(this._rledetect._idmk_list.getItem(i_id &MASK_IDNUM)).tmat;
			}
		}
		/**
		...
		public function getMarkerVertex2D(i_id:int):Vector.<NyARIntPoint2d>
		{
			if((i_id & MASK_IDTYPE)==IDTYPE_ARTK){
				//ARマーカ
				return MarkerInfoARMarker(this._rledetect._armk_list.getItem(i_id &MASK_IDNUM)).tl_vertex;
			}else{
				//Idマーカ
				return MarkerInfoNyId(this._rledetect._idmk_list.getItem(i_id &MASK_IDNUM)).tl_vertex;
			}
		}
		/**
		 * この関数は、2値化敷居値を設定します。
		...
		private var _hist_th:INyARHistogramAnalyzer_Threshold;
		/**
...
{
	private const INITIAL_MARKER_STACK_SIZE:int=10;
	private var _coordline:NyARCoord2Linear;
	
	private var _sq_stack:SquareStack;
	public var lost_th:int=5;	
	public var _tracking_list:TrackingList;
	public var _armk_list:ARMarkerList;
	public var _idmk_list:NyIdList;
	public var _transmat:INyARTransMat;
	private var _ref_input_rfb:INyARPerspectiveCopy ;
	private var _ref_input_gs:INyARGrayscaleRaster;
	public function RleDetector(i_config:INyARMarkerSystemConfig)
	{
	...
	protected override function onSquareDetect(i_coord:NyARIntCoordinates,i_vertex_index:Vector.<int>):void
	{
		...
		//観測座標点の記録
		for(i2=0;i2<4;i2++){
			sq_tmp.ob_vertex[i2].setValue_1(i_coord.items[i_vertex_index[i2]]);
		}
		//頂点分布を計算
		...
		if(is_target_marker){
			...
			for(i2 = 0; i2 < 4; i2++) {
				//直線同士の交点計算
				if(!sq_tmp.line[i2].crossPos_1(sq_tmp.line[(i2 + 3) % 4],sq_tmp.sqvertex[i2])){
	...
	public function detectMarker(i_sensor:NyARSensor,i_time_stamp:int , i_th:int):void
	{
		var i:int;
		...
		super.detectMarker_2(this._ref_input_gs,i_th);
		...
		for(i=this._armk_list.size()-1;i>=0;i--){
			...
			if(target1.lost_count==0){
				target1.time_stamp=i_time_stamp;
		...
		for(i=this._idmk_list.size()-1;i>=0;i--){
			...
			if(target2.lost_count==0){
				target2.time_stamp=i_time_stamp;
